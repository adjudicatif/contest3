...
class RegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        for i, (W, b) in enumerate(zip(self.Ws, self.bs)):
          ...
          #The ReLu is only between the hidden layers so not on the last
          if i < len(self.Ws) - 1:
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        graph = Graph(self.Ws + self.bs)
        X = Input(graph, x)
        X_1 = MatrixMultiply(graph, X, Input(graph, np.array([[1.0]])))
        X_2 = MatrixMultiply(graph, X, Input(graph, np.array([[-1.0]])))
        for i, (W, b) in enumerate(zip(self.Ws, self.bs)):
          XW_1 = MatrixMultiply(graph, X_1, W)
          XW_plus_b_1 = MatrixVectorAdd(graph, XW_1, b)
          XW_2 = MatrixMultiply(graph, X_2, W)
          XW_plus_b_2 = MatrixVectorAdd(graph, XW_2, b)
          #The ReLu is only between the hidden layers so not on the last
          if i < len(self.Ws) - 1:
            X_1 = ReLU(graph, XW_plus_b_1)
            X_2 = ReLU(graph, XW_plus_b_2)
          else:
            X_1 = XW_plus_b_1
            X_2 = XW_plus_b_2
        X_1 = MatrixMultiply(graph, X_1, Input(graph, np.array([[0.5]])))
        X_2 = MatrixMultiply(graph, X_2, Input(graph, np.array([[-0.5]])))
        X = Add(graph, X_1, X_2)
        if y is not None:
            # At training time, the correct output `y` is known.
            # Here, you should construct a loss node, and return the nn.Graph
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.d = 60
        self.a = 60
        self.H0 = Variable(self.d)
        #Hidden layer inside
        self.hs = [self.d]#at least self.d in the end to output an H
        self.Ws, self.bs = [], []
        i = self.a
        for h in self.hs:
          self.Ws.append(Variable(i, h))
          self.bs.append(Variable(h))
          i = h
        #Merge of the H and C 
        self.V = Variable(self.d, self.a)
        self.W = Variable(self.num_chars, self.a)
        self.b = Variable(self.a)
        #Hidden Layers outside
        self.hs2 = [len(self.languages)]
        self.Ws2, self.bs2 = [], []
        i = self.d
        for h in self.hs2:
          self.Ws2.append(Variable(i, h))
          self.bs2.append(Variable(h))
          i = h
    ...
    def run(self, xs, y=None):
        ...
        "*** YOUR CODE HERE ***"
        graph = Graph(self.Ws + self.bs + self.Ws2 + self.bs2 + [self.H0, self.V, self.W, self.b])
        H = MatrixVectorAdd(graph, Input(graph, np.zeros((batch_size, self.d))), self.H0)
        H = Input(graph, np.zeros((batch_size, self.d)))
        for i, x in enumerate(xs):
          #First compute a common vector
          C = Input(graph, x)
          HV = MatrixMultiply(graph, H, self.V)
          CW = MatrixMultiply(graph, C, self.W)
          HW_CW = Add(graph, HV, CW)
          HW_CW_b = MatrixVectorAdd(graph, HW_CW, self.b)
          X = ReLU(graph, HW_CW_b)
          ...
          for i, (W, b) in enumerate(zip(self.Ws, self.bs)):
            ...
            if i < len(self.Ws) - 1:
              ...
              H = XW_plus_b
        #Then add the hidden layers for the other part
        X = H
        for i, (W, b) in enumerate(zip(self.Ws2, self.bs2)):
            XW = MatrixMultiply(graph, X, W)
            XW_plus_b = MatrixVectorAdd(graph, XW, b)
#            X = ReLU(graph, XW_plus_b)
            if i < len(self.Ws) - 1:
