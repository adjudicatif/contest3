...
class Graph(object):
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        #init the gradient of the loss to 1
        ...
        for node in reversed(self.nodes):
          ...
          if hasattr(node, "parents"):
            for parent, gradient in zip(node.parents, gradients):
              self.gradients[parent] += gradient
...
#          print(self.get_gradient(v))
          v.data -= self.get_gradient(v) * step_size
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        [x] = inputs
        return [gradient * np.where(x > 0, np.ones_like(x), np.zeros_like(x))]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        [a, b] = inputs
        M = np.prod(a.shape)
        return [gradient * (a - b) / M, - gradient * (a - b) / M]
